...
{
    double n3 = 0.01, nmrk = 0;
    while (n2 <= 64)
    {
        uint64_t mk = 0, tptm = 0, mnk = 0, x = 0, dad = 0;
        ...
        //if (((x + tptm + mnk) <= n1+65535) || ((x + tptm + mnk) >= n1-65535))
        {
            //x = pow(n1,1.0/(n3));
            uint64_t y = mnk; //pow(x, n3);
            //cout << y << " " << n2 << " " << nmrk << " " << n1 << " " << (n1-y) << endl << flush;
            return make_tuple(nmrk, y, 0);
        }
    }
...
string epic(uint64_t epiphany)
{
    ...
    bitset<64> z = epiphany;
    v = sepFix(z.to_ullong());
    z = epiphany >> 32;
    //v += sepFix(z.to_ullong());
    //z = epiphany >> 48;
    //v += sepFix(z.to_ulong());
